...
func assign_shape(shape, color, x, y) -> void:
	...
	if not (str(x).is_valid_int() and str(x).is_valid_int()):
		return
	game.assign_shape_color(shape, color, int(x), int(y))
...
func assign_random_shape(color, x, y) -> void:
	...
	if not (str(x).is_valid_int() and str(x).is_valid_int()):
		return
	game.assign_shape_color(randi_range(0, GameConstants.shape.size()-1), color, int(x), int(y))

func assign_color(color, x, y) -> void:
	if color is int:
		if color < 0 or color > GameConstants.color.size():
			return
	if not (str(x).is_valid_int() and str(x).is_valid_int()):
		return
	game.assign_color(color, int(x), int(y))
...
func assign_random_color(shape, x, y) -> void:
	...
	if not (str(x).is_valid_int() and str(x).is_valid_int()):
		return
	game.assign_shape_color(shape, randi_range(0, GameConstants.color.size()-1), int(x), int(y))

func assign_random(x, y) -> void:
	if not (str(x).is_valid_int() and str(x).is_valid_int()):
		return
	game.assign_shape_color(randi_range(0, GameConstants.shape.size()-1), randi_range(0, GameConstants.color.size()-1), int(x), int(y))

func fill_all_random() -> void:
	for x in 5:
		for y in 4:
			assign_random(x,y)
